...
int amrSkipFrames = 20;
//set skip frame for wav to amr
void SetSkipFrame(int skipframe)
{
    amrSkipFrames = skipframe;
}
...
int EncodeWAVEFileToAMRFile(const char* pchWAVEFilename, const char* pchAMRFileName, int nChannels, int nBitsPerSample)
{
    ...
    unsigned char amrFrameZero[MAX_AMR_FRAME_SIZE];
    ...
    memset(amrFrameZero, 0x0, MAX_AMR_FRAME_SIZE);
	...
	while(1)
	{
        ...
        if(frames > amrSkipFrames)
        {
            fwrite(amrFrame, sizeof (unsigned char), byte_counter, fpamr );
        }
//        else
//        {
//            fwrite(amrFrameZero, sizeof (unsigned char), byte_counter, fpamr );
//        }
	}
...
// 将AMR文件解码成WAVE文件
//int DecodeAMRFileToWAVEFile(const char* pchAMRFileName, const char* pchWAVEFilename,void errCallBack(char* errValue))
...
//	fpamr = fopen([path cStringUsingEncoding:NSASCIIStringEncoding], "rb");
    fpamr = fopen(pchAMRFileName, "rb");//"rb"是以二进制的形式读
    
	if ( fpamr==NULL )
    {
       // errCallBack(strerror(err));
        return 0;
    }
...
//	fpwave = fopen([docFilePath cStringUsingEncoding:NSASCIIStringEncoding], "wb");
    fpwave = fopen(pchWAVEFilename,"wb");//"wb"是以二进制的形式写
